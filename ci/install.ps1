...
# Cached-install skip: when the deploy job restores a warm PREFIX
# cache (actions/cache on installs/, keyed on this script + Botan
# version), the entire C stack below is already in place — skip
# ~30 min of builds. The epilogue (RNP_* / RUSTFLAGS exports,
# sanity checks) still runs unconditionally.
$stackCacheHit = (Test-Path (Join-Path $Env:PREFIX "lib/botan-3.lib")) -and `
                 (Test-Path (Join-Path $Env:PREFIX "lib/rnp.lib"))
if (-not $stackCacheHit) {
