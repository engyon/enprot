...
# Skip cmake --install — it tries to install rnp.exe (not built).
# Manually copy headers + library to PREFIX instead.
New-Item -ItemType Directory -Force -Path "$Env:PREFIX/include/rnp" | Out-Null
Copy-Item "$WORKDIR\rnp-src\src\lib\rnp.h"        "$Env:PREFIX/include/rnp\" -ErrorAction SilentlyContinue
Copy-Item "$WORKDIR\rnp-src\src\lib\rnp_err.h"    "$Env:PREFIX/include/rnp\" -ErrorAction SilentlyContinue
# rnp_export.h is generated by cmake in the build directory.
$exportH = Get-ChildItem -Path "$WORKDIR\rnp-src\build" -Filter "rnp_export.h" -Recurse | Select-Object -First 1
if ($exportH) { Copy-Item $exportH.FullName "$Env:PREFIX/include\rnp\" }
# Find the built static library.
$rnpLib = Get-ChildItem -Path "$WORKDIR\rnp-src\build" -Filter "rnp*.lib" -Recurse | Select-Object -First 1
if ($rnpLib) { Copy-Item $rnpLib.FullName "$Env:PREFIX\lib\" }
# Also copy sexpp.lib if present.
$sexppLib = Get-ChildItem -Path "$WORKDIR\rnp-src\build" -Filter "sexpp*.lib" -Recurse | Select-Object -First 1
if ($sexppLib) { Copy-Item $sexppLib.FullName "$Env:PREFIX\lib\" }
