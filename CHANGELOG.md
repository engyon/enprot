...
- *(escrow)* OpenPGP recipients — --pgp-pubkey wraps the CEK to PGP keys via librnp
- *(cli)* turnkey git wiring — enprot init --git configures the full filter/diff/merge trio

### Fixed

- fpr -> fp — the typos gate prefers the codebase's existing spelling
...
- bump sha3 from 0.10.9 to 0.12.0
...
- verification + mobile scaffolding (TODOs 64 + 47)
...
- *(site)* reflect 0.5.62 — IPFS CAS on the shipped list and in the audit guide

## [0.5.62](https://github.com/engyon/enprot/compare/enprot-v0.5.61...enprot-v0.5.62) - 2026-08-24

### Added

- *(cas)* IPFS backend behind cas-ipfs, live-tested against Kubo in CI
...
- *(site)* reflect 0.5.61 — S3 CAS, OTLP observability, fuzzing
