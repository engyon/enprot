...
(Botan 3.x; CI builds against 3.7.0, Homebrew ships 3.12.0).
...
For contributors, a pre-commit hook mirrors the CI checks (format, clippy,
spell check). Install it once after cloning:
...
CI also runs `cargo-deny` (license + security advisories via `deny.toml`)
and `cargo-audit` (RustSec vulnerability database) on every push.
...
Usage: enprot [OPTIONS] <COMMAND>

Commands:
  encrypt        Encrypt WORD segments in the input file(s)
  decrypt        Decrypt WORD segments in the input file(s)
  store          Store (unencrypted) WORD segments to CAS
  fetch          Fetch (unencrypted) WORD segments from CAS
  encrypt-store  Encrypt and store WORD segments
  passthrough    Parse and re-write without applying any transform
  help           Print this message or the help of the given subcommand(s)

Options:
  -v, --verbose            Produce more verbose output
  -q, --quiet              Suppress unnecessary output
  -l, --left-separator <SEP>    Specify left separator [default: "// <("]
  -r, --right-separator <SEP>   Specify right separator [default: ")>"]
  -k, --key <WORD=PASSWORD>     Specify a secret PASSWORD for WORD (repeatable)
  -c, --casdir <DIRECTORY>      Directory for CAS files (default "cas" if exists)
      --policy <POLICY>         Restrict algorithms [default: default]
      --fips                    Force FIPS-compliant algorithms (implies --policy=nist)
  -h, --help                   Print help
  -V, --version                Print version

Run `enprot <command> --help` for command-specific options (e.g. `enprot
encrypt --help` shows the PBKDF and cipher knobs).
----

Each subcommand takes the WORD list via `-w`/`--word` (repeatable or
comma-separated) and the input files as positional args. For example:

[source,sh]
----
enprot encrypt -w Agent_007 -k Agent_007=bond sample/test.ept
enprot decrypt -w Agent_007 -k Agent_007=bond sample/test.ept
enprot store  -w Agent_007,GEHEIM sample/test.ept
enprot fetch  -w Agent_007,GEHEIM sample/test.ept
...
[source,sh]
----
enprot$ ./target/debug/enprot passthrough sample/test.ept
enprot$
----
...
[source,sh]
----
enprot$ ./target/debug/enprot passthrough sample/test.ept -v
...
----
enprot$ ./target/debug/enprot store -w GEHEIM sample/test.ept
...
[source,sh]
----
enprot$ ./target/debug/enprot fetch -w GEHEIM sample/test.ept
----
...
[source,sh]
----
enprot$ ./target/debug/enprot fetch -w Agent_007,notexistent,GEHEIM sample/test.ept
----
...
[source,sh]
----
enprot$ ./target/debug/enprot encrypt -w Agent_007 sample/test.ept
...
[source,sh]
----
enprot$ ./target/debug/enprot encrypt -w GEHEIM -k GEHEIM=james
enprot$ cat sample/test.ept
hello, this is a test file
// <( ENCRYPTED GEHEIM )>
...
[source,sh]
----
enprot$ ./target/debug/enprot decrypt -w Agent_007,GEHEIM -k GEHEIM=james -k Agent_007=bond
enprot$ cat sample/test.ept
hello, this is a test file
// <( BEGIN GEHEIM )>
Secret line 1
...
[source,sh]
----
enprot$ ./target/debug/enprot decrypt -w AUTHOR -k AUTHOR=markku src/lib.rs
enprot$ cargo run -- -h
...
[source,sh]
----
enprot$ ./target/debug/enprot encrypt-store -w GEHEIM sample/test.ept
Password for GEHEIM:
...
[source,sh]
----
enprot$ ./target/debug/enprot decrypt -w GEHEIM sample/test.ept
Password for GEHEIM:
enprot$
----
...
[source,sh]
----
enprot$ ./target/debug/enprot passthrough input.ept -o output.ept
----
...
----
enprot$ ./target/debug/enprot passthrough -p output/ file.*
----
...
with Associated Data (AEAD) mechanism is used. The default is
...
The cipher backend is split:

[cols="1,2,1", options="header"]
|===
| Algorithm | Backend | Notes
| `aes-256-siv` (default)   | link:https://botan.randombit.net/[Botan 3]            | RFC 5297, 64-byte double key (derived from PBKDF).
| `aes-256-gcm`             | link:https://botan.randombit.net/[Botan 3]            | NIST policy default; 96-bit IV.
| `aes-256-gcm-siv`         | link:https://github.com/RustCrypto/AEADs[aes-gcm-siv (RustCrypto)] | RFC 8452; Botan does not implement this AEAD.
|===

All hash function computations for CAS utilize SHA-3 [FIPS202] variants.
SHA-3 is also used to derive keying material from passwords.

Supported PBKDFs: `argon2` (default-policy default), `scrypt`,
`pbkdf2-sha256`, `pbkdf2-sha512`, and `legacy`. The `legacy` PBKDF is
plain SHA3-512 truncation with no salt or work factor; it is retained
only so documents encrypted by very old enprot releases can still be
decrypted. Selecting it for new encryption prints a warning.
...
[source]
----
// <( ENCRYPTED <keyword> pbkdf:$<id>$<k=v,k=v>$<base64-salt> cipher:$<alg>$iv=<base64-iv> )>
----
